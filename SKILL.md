...
name: h5i-dispatch
version: 0.1.1
description: 把一个隔离、可并行的子任务派给另一个 agent(codex/claude/ 或任意 Claude-Code-harness)后台执行，经 h5i(基于 git ref 的消息总线)+ git worktree 通信，worker 跑完自动唤主 session 验证回报。显式触发：派给另一个 agent、并行做 X、dispatch、后台让 codex/claude 去做、让另一个 agent 写 X、agent 通信派活、并行派一个 worker。**主动识别(不必等用户明说)**：当 agent 发现手头活可拆成 ≥2 个互不相干、可隔离的子任务——跨端补齐 / 补多端单测 / 修 N 个独立用例 / 面临"先做 A 还是 B"的独立任务分叉 / 长耗时隔离子活——应主动提示用户"这块可并行派给 worker"(只提示不自动派)。
---
...
| **worker CLI** | codex（独立配额）/ claude（`-p` headless）/ 任意 Claude-Code-harness 跑别的后端（如 Kimi）。在 PATH 或设 `CODEX_BIN`/`CLAUDE_BIN` |
...
| `CODEX_BIN` / `CLAUDE_BIN` | `command -v` 解析 | worker 二进制路径 |
...
**一把梭**：`scripts/dispatch.sh <codex|claude> <wt-name> <task-file> [dispatcher] [worker-id]`（封装下面 1–4；`wt-name` 限 `^[A-Za-z0-9._-]+$`，`worker-id` 默认 `<kind>-<wt-name>` **唯一**，避免并发派发身份串线）。
...
- 干完 `H5I_AGENT=<worker> "$H5I_BIN" msg send <dispatcher> "DONE: <summary>"` 回报（用 `send` 不用 `done <N>`：并发派发时编号会错位）
- "只动 <path>、自主、别问问题"

启动（`$WBIN`=worker 二进制，`$WT`=worktree 绝对路径）：
- **codex**：`[DISPATCH_PROXY 时 export HTTP(S)_PROXY+ALL_PROXY 大小写] H5I_AGENT=<worker> gtimeout 1800 "$WBIN" exec --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check -C "$WT" -o <last.txt> - < <prompt>`
- **claude**：`cd "$WT" && gtimeout 1800 "$WBIN" -p "<prompt>" --allowedTools Read,Grep,Glob,Edit,Bash`（claude 无 `-C`，**必须 `cd "$WT"`**，否则在主树写文件）
...
- **`--dangerously-bypass-approvals-and-sandbox` = 给 worker 整机读写+执行权**（只为写共享 `.git` 的 refs/h5i，影响面远超此需）
